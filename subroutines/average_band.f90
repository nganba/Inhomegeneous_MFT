...
program average
  ...
  character(5):: junk
  real(kind=double),dimension(:),allocatable::omega,dostmp,dos,kpts


  open(unit=1,file='names.dat',status='OLD')
  open(unit=11,file='avgband.dat',status='NEW')
   ...
   i=0
   do
     read(2,*,end=10) junk,junk
     i=i+1
   enddo
10 L=i
   close(2)
   rewind(1)
   allocate(dos(L),dostmp(L),omega(L),kpts(L),stat=INFO)
   if(INFO.ne.0)then
...
20 read(1,*,end=30) fname
   ...
   open(unit=2,file=fname,status='OLD')
   do i=1,L
     read(2,*) kpts(i),omega(i),dostmp(i)
   enddo
   dos=dos+dostmp
   close(2)
   goto 20
