...
program spectral_function
  ...
  integer,parameter::double = selected_real_kind(p=13,r=200)
  real(kind=double),parameter::pi=acos(-1.0)
  integer:: L,M,seed,nsite,i,j,k,nk,n,nkpts
  real(kind=double)::U,dV,t,omega,eta,mu,beta,nup,qx,qy,x1,x2,y1,y2,tmp2,Akw,dis,theta,phi,phasediv
  complex(kind=double)::tmp
  real(kind=double),dimension(:,:),allocatable::xyz,bz,basis
  ...
  integer,dimension(:),allocatable::mag_basis
  ...
  read(2,*) nsite
  close(2)
  allocate(xyz(nsite,3),V(nsite),eval(2*nsite),field(3*nsite),ham(nsite,nsite),hmat(2*nsite,2*nsite),mag_basis(nsite),basis(3,3))
  ...
  !Identify basis atoms inside the magnetic unit cell
  call identify_magnetic_basis(mag_basis)
  !do i=1,nsite
  !   write(10,*) xyz(i,1),xyz(i,2),mag_basis(i)
  !enddo
  !goto 10
  ...
  do phi=0d0,2.d0*pi-pi/phasediv,2.d0*pi/phasediv
     ...
     do nk=1,nkpts
        ...
        do n=1,2*nsite
           ...
           if(eval(n).lt.-1.d0 .or. eval(n).gt.0d0) cycle
           tmp=0d0
           do i=1,nsite
              do j=1,nsite
                 if(mag_basis(i).ne.mag_basis(j)) cycle
                 x1=xyz(i,1)
                 y1=xyz(i,2)
                 x2=xyz(j,1)
                 y2=xyz(j,2)
              ...
              enddo
           enddo
           Akw = Akw + real(tmp)/nsite !*lorentz(omega-eval(n),eta)/nsite
        enddo
        write(1,*) qx,qy,Akw
     enddo 
