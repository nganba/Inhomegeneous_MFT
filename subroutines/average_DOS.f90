...
program average
  ...
  character(5):: junk
  real(kind=double),dimension(:),allocatable::omega,dostmp,dos


  open(unit=1,file='names.dat',status='OLD')
  open(unit=11,file='avgDOS.dat',status='NEW')
...
10 L=i
   close(2)
   rewind(1)
   allocate(dos(L),dostmp(L),omega(L),stat=INFO)
   if(INFO.ne.0)then
